...
import com.example.flightservice.exception.GlobalExceptionHandler;
...
class GlobalExceptionHandlerTest {
  ...
  void shouldHandleMethodArgumentNotValid() {
    ...
    FieldError fieldError = new FieldError("object", "origin", null, false, null, null, "Origin is required");
    ...
    assertThat(response.getBody().getError().getValidationErrors().get(0).getField()).isEqualTo("origin");
  }
  ...
  void shouldHandleBindException() {
    ...
    FieldError fieldError = new FieldError("object", "destination", null, false, null, null, "Destination is required");
    when(bindingResult.getFieldErrors()).thenReturn(List.of(fieldError));
    when(ex.getBindingResult()).thenReturn(bindingResult);
  ...
  void shouldHandleConstraintViolation() {
    ...
    when(path.toString()).thenReturn("departureDate");
    when(violation.getPropertyPath()).thenReturn(path);
    when(violation.getMessage()).thenReturn("must be in the future");
    when(violation.getInvalidValue()).thenReturn("2020-01-01");
